...
async def check_routing(state: LegalState) -> dict:
    """Routing node: determine which specialist sub-agents are needed using keyword matching."""
    ...
    needs_privacy = any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "ccpa", "personal"])
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        ...
        "Phân tích các vấn đề về privacy, GDPR, CCPA và quyền bảo vệ dữ liệu. "
        "Tập trung vào: GDPR fines (up to 4% global revenue), CCPA violations, "
        "data breach notification requirements, và individual rights. "
...
async def main():
    ...
    # Bước 3: Vẽ graph (codelab yêu cầu)
    try:
        from IPython.display import Image, display
        display(Image(graph.get_graph().draw_mermaid_png()))
        print("[Graph] Đã hiển thị graph trong notebook.")
    except Exception:
        try:
            png_data = graph.get_graph().draw_mermaid_png()
            with open("stage4_graph.png", "wb") as f:
                f.write(png_data)
            print("[Graph] Đã lưu graph vào file: stage4_graph.png")
        except Exception as e:
            print(f"[Graph] Không thể vẽ graph: {e}")
